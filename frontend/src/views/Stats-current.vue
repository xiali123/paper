<template>
  <div class="stats-page">
    ...
    <!-- Stats Content -->
    <div v-else-if="stats.overview" class="stats-content">
      <!-- Key Metrics Section -->
      <div class="metrics-section">
        <div class="metrics-grid">
          <div class="metric-card card card-compact">
            ...
            <div class="metric-value">{{ stats.overview.totalPapers?.toLocaleString() || '0' }}</div>
          ...
          <div class="metric-card card card-compact">
            ...
            <div class="metric-value">{{ stats.overview.totalJournals?.toLocaleString() || '0' }}</div>
          ...
          <div class="metric-card card card-compact">
            ...
            <div class="metric-value">{{ stats.overview.topTierPapers?.toLocaleString() || '0' }}</div>
          ...
          <div class="metric-card card card-compact">
            ...
            <div class="metric-value">{{ stats.overview.papersLastYear?.toLocaleString() || '0' }}</div>
    ...
    <div v-else class="error-state">
      <div class="error-card card card-spacious">
        ...
        <div class="debug-info" v-if="!stats.loading && !stats.error">
          <p>Debug Info:</p>
          <p>loading: {{ stats.loading }}</p>
          <p>hasData: {{ stats.hasData }}</p>
          <p>overview: {{ stats.overview }}</p>
        </div>
      </div>
    </div>
  </div>
...
// 组件挂载时加载数据
onMounted(async () => {
  console.log('🎯 [Stats.vue] Component mounted, starting data fetch...')
  console.log('📊 [Stats.vue] Initial stats state:', {
  ...
  console.log('🔍 [Stats.vue] Calling fetchOverview...')

  try {
    await stats.fetchOverview()
    console.log('✅ [Stats.vue] fetchOverview completed')
  } catch (err) {
    console.error('❌ [Stats.vue] fetchOverview failed:', err)
  }

  console.log('📊 [Stats.vue] Final stats state:', {
    loading: stats.loading,
    hasData: stats.hasData,
    error: stats.error,
    overview: stats.overview
