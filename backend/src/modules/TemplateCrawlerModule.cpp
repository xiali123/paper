...
std::optional<CrawlerTemplate> CrawlerTemplate::fromJson(const std::string& json) {
    try {
        ...
        if (j.contains("sourceType")) j["sourceType"].get_to(tmpl.sourceType);
        ...
        if (j.contains("fieldRules") && j["fieldRules"].is_object()) {
            for (auto& [fieldName, fieldRuleJson] : j["fieldRules"].items()) {
                FieldRule rule;
                if (fieldRuleJson.contains("ruleType")) fieldRuleJson["ruleType"].get_to(rule.ruleType);
        ...
        auto logging = spdlog::get("TemplateCrawler");
        if (logging) {
            logging->error("Failed to parse CrawlerTemplate from JSON: {}", e.what());
        }
        return std::nullopt;
    }
}
