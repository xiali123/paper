...
PaperReadingVoyage::PaperReadingVoyage(QWidget* parent)
    : QWidget(parent)
    , settings_("PaperCrawler", "ReadingVoyage")
    , sailBtn_(nullptr)
    , clearBtn_(nullptr)
...
void PaperReadingVoyage::setupUI()
{
    ...
    inputField_->setPlaceholderText(tr("Enter destination name..."));
    ...
    connect(sailBtn_, &QPushButton::clicked, this, &PaperReadingVoyage::onSail);
    connect(clearBtn_, &QPushButton::clicked, this, &PaperReadingVoyage::onClear);
}

void PaperReadingVoyage::addEntry(const VoyageEntry& entry)
{
...
QList<VoyageEntry> PaperReadingVoyage::entries() const
{
    return entries_;
}

int PaperReadingVoyage::arrivedCount() const
{
    int count = 0;
...
qreal PaperReadingVoyage::avgDistance() const
{
    if (entries_.isEmpty())
        return 0.0;
    qreal sum = 0.0;
...
QMap<QString, int> PaperReadingVoyage::categoryCounts() const
{
...
void PaperReadingVoyage::onSail()
{
    auto* rng = QRandomGenerator::global();
    const int count = rng->bounded(3, 7);

    const QStringList vessels = {"Frigate", "Galleon", "Schooner", "Cutter", "Dreadnought"};

    for (int i = 0; i < count; ++i) {
        VoyageEntry entry;
        entry.id = static_cast<int>(QDateTime::currentMSecsSinceEpoch() % 100000) + i;
        entry.destination = inputField_->text().trimmed().isEmpty()
            ? QStringLiteral("Port-%1").arg(entries_.size() + 1)
            : inputField_->text().trimmed();
        entry.category = categoryCombo_->currentText();
        entry.vessel = vessels.at(rng->bounded(static_cast<int>(vessels.size())));
        ...
        entries_.append(entry);
        emit voyageComplete(entry.id, entry.distance);
    }
...
void PaperReadingVoyage::onClear()
{
    entries_.clear();
    updateInfo();
    saveSettings();
...
void PaperReadingVoyage::paintEvent(QPaintEvent*)
{
    QPainter p(this);
    ...
    const int controlH = (sailBtn_ ? sailBtn_->geometry().bottom() : 0)
    ...
    drawVoyageMap(p, mapRect);
    drawCategoryChart(p, chartRect);
    drawStats(p, statsRect);
}

void PaperReadingVoyage::drawVoyageMap(QPainter& p, const QRect& rect)
{
    p.setPen(Qt::NoPen);
    ...
    for (int i = startIdx; i < entries_.size(); ++i) {
        ...
        const int bw = static_cast<int>(usableW * e.distance / maxDist);
        ...
        p.drawText(QRect(rect.x() + leftMargin + bw + 6, y - 1, usableW - bw, barHeight + 2),
                   ...
                   QStringLiteral("%1  %2  %3.0fmi  %4%5")
                       .arg(e.destination)
                       .arg(e.vessel)
                       .arg(e.distance)
                       .arg(e.ports)
                       .arg(e.arrived ? " [arrived]" : ""));

        y += barHeight + gap;
    }
}

void PaperReadingVoyage::drawCategoryChart(QPainter& p, const QRect& rect)
{
    p.setPen(Qt::NoPen);
    p.setBrush(QColor("#ffffff"));
    p.drawRoundedRect(rect, 8, 8);
...
void PaperReadingVoyage::drawStats(QPainter& p, const QRect& rect)
{
    p.setPen(Qt::NoPen);
    p.setBrush(QColor("#ffffff"));
    p.drawRoundedRect(rect, 8, 8);
    ...
    const int totalPorts = [&]() {
        int s = 0;
        for (const auto& e : entries_) s += e.ports;
        return s;
    }();
    ...
    drawLine(tr("Total voyages:"), QString::number(entries_.size()));
    drawLine(tr("Arrived:"), QString::number(arrivedCount()));
    drawLine(tr("Avg distance:"), QString::number(avgDistance(), 'f', 1) + " mi");
    drawLine(tr("Total ports:"), QString::number(totalPorts));
}

void PaperReadingVoyage::updateInfo()
{
    if (entries_.isEmpty()) {
    ...
    infoLabel_->setText(tr("%1 voyage(s) | %2 arrived | %3 port(s) | avg distance %4 mi")
        .arg(entries_.size())
        .arg(arrivedCount())
        .arg(totalPorts)
...
void PaperReadingVoyage::loadSettings()
{
...
void PaperReadingVoyage::saveSettings()
{
    settings_.beginGroup("entries");
