...
PaperEvidenceEvaluator::PaperEvidenceEvaluator(QWidget* parent)
    : QWidget(parent)
    , settings_("PaperCrawler", "EvidenceEvaluator")
{
    setupUI();
    loadSettings();
}

void PaperEvidenceEvaluator::setupUI() {
    ...
    connect(evalBtn_, &QPushButton::clicked, this, &PaperEvidenceEvaluator::onEvaluate);
    toolbar->addWidget(evalBtn_);
    toolbar->addWidget(new QLabel("Type:"));
    typeCombo_ = new QComboBox();
    typeCombo_->addItems({"All", "Empirical", "Theoretical", "Experimental", "Statistical"});
    toolbar->addWidget(typeCombo_, 1);
    clearBtn_ = new QPushButton("Clear");
    clearBtn_->setStyleSheet("color: #dc2626;");
    connect(clearBtn_, &QPushButton::clicked, this, &PaperEvidenceEvaluator::onClear);
    ...
    infoLabel_ = new QLabel("Evaluate evidence quality");
    infoLabel_->setStyleSheet("font-size: 12px; color: #334155; padding: 4px;");
    layout->addWidget(infoLabel_);
    setMinimumSize(580, 480);
}

void PaperEvidenceEvaluator::addEntry(const EvidenceEntry& entry) {
...
QList<EvidenceEntry> PaperEvidenceEvaluator::entries() const { return entries_; }

int PaperEvidenceEvaluator::verifiedCount() const {
    int c = 0;
    for (const auto& e : entries_) if (e.verified) c++;
    return c;
}

qreal PaperEvidenceEvaluator::avgStrength() const {
...
QMap<QString, int> PaperEvidenceEvaluator::typeCounts() const {
    QMap<QString, int> counts;
    for (const auto& e : entries_) counts[e.type]++;
    return counts;
}

void PaperEvidenceEvaluator::onEvaluate() {
    QString text = inputField_->text().trimmed();
    if (text.isEmpty()) return;
    QStringList types = {"empirical", "theoretical", "experimental", "statistical"};
    QStringList sources = {"paper-A", "paper-B", "dataset-C", "survey-D"};
    ...
    for (int i = 0; i < count; ++i) {
        ...
        e.reliability = 0.4 + QRandomGenerator::global()->bounded(60) / 100.0;
        e.verified = e.strength >= 0.7 && e.reliability >= 0.6;
        e.color = e.verified ? QColor(16,185,129) : (e.strength >= 0.5 ? QColor(59,130,246) : QColor(239,68,68));
        addEntry(e);
    }
    inputField_->clear();
}

void PaperEvidenceEvaluator::onClear() {
    entries_.clear();
    saveSettings();
    infoLabel_->setText("Evaluate evidence quality");
    update();
}

void PaperEvidenceEvaluator::paintEvent(QPaintEvent*) {
    QPainter p(this);
    ...
    if (entries_.isEmpty()) {
        ...
        p.drawText(rect(), Qt::AlignCenter, "Evaluate evidence quality");
        return;
    }
    p.setPen(QColor(15, 23, 42));
    p.setFont(QFont("Arial", 13, QFont::Bold));
    p.drawText(20, 30, "Evidence Evaluator");
...
void PaperEvidenceEvaluator::drawEvidenceList(QPainter& p, const QRect& rect) {
...
void PaperEvidenceEvaluator::drawTypeChart(QPainter& p, const QRect& rect) {
    p.setPen(QColor(100, 116, 139));
    p.setFont(QFont("Arial", 10));
    p.drawText(rect.topLeft(), "Evidence Types");
    auto counts = typeCounts();
    QStringList types = {"empirical", "theoretical", "experimental", "statistical"};
    QString labels[] = {"Empirical", "Theory", "Exper.", "Stats"};
...
void PaperEvidenceEvaluator::drawStats(QPainter& p, const QRect& rect) {
...
void PaperEvidenceEvaluator::updateInfo() {
    if (entries_.isEmpty()) { infoLabel_->setText("Evaluate evidence quality"); return; }
    infoLabel_->setText(QString("%1 evidence | %2 verified | %3% avg")
        .arg(entries_.size()).arg(verifiedCount()).arg(avgStrength() * 100, 0, 'f', 0));
}

void PaperEvidenceEvaluator::loadSettings() {
...
void PaperEvidenceEvaluator::saveSettings() {
