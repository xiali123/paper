...
PaperLogicChain::PaperLogicChain(QWidget* parent)
    : QWidget(parent)
    , settings_("PaperCrawler", "LogicChain")
{
    setupUI();
    loadSettings();
}

void PaperLogicChain::setupUI() {
    ...
    connect(validateBtn_, &QPushButton::clicked, this, &PaperLogicChain::onValidate);
    ...
    inputField_->setPlaceholderText("Enter logic chain text...");
    inputField_->setStyleSheet("QLineEdit { padding: 6px; border: 1px solid #cbd5e1; border-radius: 4px; }");
    toolbar->addWidget(inputField_, 2);
    clearBtn_ = new QPushButton("Clear");
    clearBtn_->setStyleSheet("color: #dc2626;");
    connect(clearBtn_, &QPushButton::clicked, this, &PaperLogicChain::onClear);
    toolbar->addWidget(clearBtn_);
    layout->addLayout(toolbar);
    infoLabel_ = new QLabel("Validate logic chains");
    infoLabel_->setStyleSheet("font-size: 12px; color: #334155; padding: 4px;");
    layout->addWidget(infoLabel_);
    setMinimumSize(580, 480);
}

void PaperLogicChain::addEntry(const LogicEntry& entry) {
    entries_.append(entry);
    saveSettings();
    updateInfo();
    emit chainValidated(entry.id, entry.validity);
    update();
}

QList<LogicEntry> PaperLogicChain::entries() const { return entries_; }

int PaperLogicChain::soundCount() const {
    int c = 0;
    for (const auto& e : entries_) if (e.sound) c++;
    return c;
}

qreal PaperLogicChain::avgValidity() const {
...
QMap<QString, int> PaperLogicChain::categoryCounts() const {
    QMap<QString, int> counts;
    for (const auto& e : entries_) counts[e.category]++;
    return counts;
}

void PaperLogicChain::onValidate() {
    ...
    for (int i = 0; i < count; ++i) {
        ...
        e.conclusion = "conc" + QString::number(i);
        e.validity = 0.3 + QRandomGenerator::global()->bounded(70) / 100.0;
...
void PaperLogicChain::onClear() {
    entries_.clear();
    saveSettings();
    infoLabel_->setText("Validate logic chains");
    update();
}

void PaperLogicChain::paintEvent(QPaintEvent*) {
    QPainter p(this);
    ...
    if (entries_.isEmpty()) {
        ...
        p.drawText(rect(), Qt::AlignCenter, "Validate logic chains");
        return;
    }
    p.setPen(QColor(15, 23, 42));
    p.setFont(QFont("Arial", 13, QFont::Bold));
    p.drawText(20, 30, "Logic Chain");
...
void PaperLogicChain::drawChainDiagram(QPainter& p, const QRect& rect) {
    ...
    for (int i = 0; i < show; ++i) {
        ...
        p.drawText(rect.x() + 10, y + 4, rect.width() / 2 - 10, 16, Qt::AlignVCenter,
                   e.premise.left(10) + " -> " + e.conclusion.left(6) + (e.sound ? " [S]" : ""));
        ...
        p.drawText(rect.x() + rect.width() / 2, y + 20, rect.width() / 2 - 10, 14,
                   Qt::AlignVCenter | Qt::AlignRight,
                   e.sound ? "sound" : "unsound");
    }
}

void PaperLogicChain::drawCategoryChart(QPainter& p, const QRect& rect) {
    p.setPen(QColor(100, 116, 139));
...
void PaperLogicChain::drawStats(QPainter& p, const QRect& rect) {
    struct Stat { QString label; QString value; QColor color; };
    QList<Stat> stats = {
        {"Chains", QString::number(entries_.size()), QColor(59,130,246)},
        {"Sound", QString::number(soundCount()), QColor(22,163,74)},
        {"Avg Val", QString::number(avgValidity() * 100, 'f', 0) + "%", QColor(217,119,6)},
...
void PaperLogicChain::updateInfo() {
    if (entries_.isEmpty()) { infoLabel_->setText("Validate logic chains"); return; }
    infoLabel_->setText(QString("%1 chains | %2 sound | %3% validity")
        .arg(entries_.size()).arg(soundCount()).arg(avgValidity() * 100, 0, 'f', 0));
}

void PaperLogicChain::loadSettings() {
...
void PaperLogicChain::saveSettings() {
