...
PaperMilestoneTracker::PaperMilestoneTracker(QWidget* parent)
    : QWidget(parent)
    , settings_("PaperCrawler", "MilestoneTracker")
{
    setupUI();
    loadSettings();
}

void PaperMilestoneTracker::setupUI() {
    ...
    connect(createBtn_, &QPushButton::clicked, this, &PaperMilestoneTracker::onCreate);
    ...
    connect(clearBtn_, &QPushButton::clicked, this, &PaperMilestoneTracker::onClear);
...
void PaperMilestoneTracker::addEntry(const MilestoneEntry& entry) {
    entries_.append(entry);
    saveSettings();
    updateInfo();
    emit milestoneCreated(entry.id, entry.progress);
    update();
}

QList<MilestoneEntry> PaperMilestoneTracker::entries() const { return entries_; }

int PaperMilestoneTracker::criticalCount() const {
    int c = 0;
    for (const auto& e : entries_) if (e.critical) c++;
    return c;
}

qreal PaperMilestoneTracker::avgProgress() const {
...
QMap<QString, int> PaperMilestoneTracker::categoryCounts() const {
    QMap<QString, int> counts;
    for (const auto& e : entries_) counts[e.category]++;
    return counts;
}

void PaperMilestoneTracker::onCreate() {
    QString text = inputField_->text().trimmed();
    if (text.isEmpty()) return;
    QStringList categories = {"research", "writing", "review", "publish"};
    QStringList statuses = {"planned", "in-progress", "blocked", "done"};
    QStringList deps = {"none", "phase-1", "phase-2", "approval"};
    int cIdx = categoryCombo_->currentIndex();
    int count = 3 + QRandomGenerator::global()->bounded(4);
    for (int i = 0; i < count; ++i) {
        MilestoneEntry e;
        e.id = entries_.size() + 1;
        e.name = text.left(8) + " ms" + QString::number(i);
        e.category = cIdx == 0 ? categories[QRandomGenerator::global()->bounded(categories.size())] : categories[cIdx - 1];
        e.status = statuses[QRandomGenerator::global()->bounded(statuses.size())];
        e.dependency = deps[QRandomGenerator::global()->bounded(deps.size())];
        e.progress = QRandomGenerator::global()->bounded(101);
        e.daysLeft = QRandomGenerator::global()->bounded(90);
        e.critical = e.daysLeft <= 7 && e.progress < 80;
        e.color = e.critical ? QColor(239,68,68) : (e.status == "done" ? QColor(16,185,129) : QColor(59,130,246));
        addEntry(e);
    }
    inputField_->clear();
}

void PaperMilestoneTracker::onClear() {
    entries_.clear();
    saveSettings();
...
void PaperMilestoneTracker::paintEvent(QPaintEvent*) {
    QPainter p(this);
...
void PaperMilestoneTracker::drawMilestoneList(QPainter& p, const QRect& rect) {
    ...
    for (int i = 0; i < show; ++i) {
        ...
        p.drawText(rect.x() + 10, y + 4, rect.width() / 2 - 10, 16, Qt::AlignVCenter,
                   e.name.left(14) + (e.critical ? " [!]" : ""));
        p.setPen(QColor(100, 116, 139));
        p.setFont(QFont("Arial", 7));
        p.drawText(rect.x() + 10, y + 20, rect.width() / 2 - 10, 14, Qt::AlignVCenter,
                   e.status + " | dep:" + e.dependency + " | " + e.category);
        // progress bar
        ...
        p.setPen(QColor(100, 116, 139));
        p.setFont(QFont("Arial", 7));
        p.drawText(barX + barW + 4, y + 14, QString::number(e.progress) + "%");
        p.drawText(barX, y + 20, barW + 30, 14, Qt::AlignVCenter,
                   QString::number(e.daysLeft) + "d left");
    }
}

void PaperMilestoneTracker::drawCategoryChart(QPainter& p, const QRect& rect) {
    p.setPen(QColor(100, 116, 139));
    p.setFont(QFont("Arial", 10));
    p.drawText(rect.topLeft(), "Categories");
    auto counts = categoryCounts();
    QStringList categories = {"research", "writing", "review", "publish"};
    QString labels[] = {"Research", "Writing", "Review", "Publish"};
...
void PaperMilestoneTracker::drawStats(QPainter& p, const QRect& rect) {
...
void PaperMilestoneTracker::updateInfo() {
    if (entries_.isEmpty()) { infoLabel_->setText("Track milestones"); return; }
    infoLabel_->setText(QString("%1 milestones | %2 critical | %3% avg")
        .arg(entries_.size()).arg(criticalCount()).arg(avgProgress(), 0, 'f', 0));
}

void PaperMilestoneTracker::loadSettings() {
...
void PaperMilestoneTracker::saveSettings() {
